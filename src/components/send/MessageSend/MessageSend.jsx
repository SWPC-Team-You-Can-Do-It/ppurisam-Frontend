...
const MessageSend = () => {
    ...
    const { isRecording: isRecordingInput, startRecording: startRecordingInput, stopRecording: stopRecordingInput } = useAudioRecorder();
    const { isRecording: isRecordingTextarea, startRecording: startRecordingTextarea, stopRecording: stopRecordingTextarea } = useAudioRecorder();
        ...
        <div className="message-send-container">
            ...
            <div className="action-buttons">
                ...
                <button
                    ...
                    onClick={isRecordingTextarea ? stopRecordingTextarea : startRecordingTextarea}
                    aria-label="녹음"
                >
                    {isRecordingTextarea ? '녹음 종료' : <img src={micIcon} alt="Mic Icon" className="mic-icon" />}
                </button>
            </div>
            <button className="submit-button1" onClick={() => window.scrollTo({ top: document.documentElement.scrollHeight, behavior: 'smooth' })}>
                작성 완료
            </button>
            ...
            <MessageCreate isOpen={isModalOpen} onClose={closeModal} onGeneratedMessage={handleGeneratedMessage} />
        </div>
    );
};
